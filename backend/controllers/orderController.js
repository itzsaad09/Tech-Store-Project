...
const placeOrder = async (req, res) => {
    try {
        ...
        const { userId, cartItemsArray, shippingFees, finalTotalBill, shippingInfo, paymentMethod, cardDetails } = req.body;

        // Prepare order data
        const orderData = {
            userId: userId,
            items: cartItemsArray, // Use cartItemsArray for items
            amount: finalTotalBill, // Use finalTotalBill for total amount
            shippingCharges: shippingFees,
            address: shippingInfo, // Use shippingInfo for address
            status: "Order Placed",
            paymentMethod: paymentMethod,
            payment: paymentMethod === 'cash_on_delivery' ? false : true, // Set payment status based on method
        };

        // If credit card is selected, add cardDetails (non-sensitive parts)
        if (paymentMethod === 'credit_card') {
            // In a real application, you would typically integrate with a payment gateway here
            // and store only a token or last 4 digits, not full card details.
        ...
        // Update user's shippingDetails with the used shippingInfo
        // This assumes you want to save the most recent shipping info used by the user
        await userModel.findByIdAndUpdate(userId, { shippingDetails: shippingInfo });
...
export { placeOrder, placeOrderStripe, verifyPayment, allOrders, updateOrderStatus, userOrders };